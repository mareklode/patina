define(['backgroundImage_helpers'], function( helpers ) {
    ...
    function backgroundImage (domElement, parameters) {
        ...
        self.noiseMap = self._createNoisemap( self._parameters.width, self._parameters.height );
        self.myCanvas = helpers.newCanvas( self._parameters.width, self._parameters.height );
            ...
            function (e) {
                var ani = self._parameters.animation,
                    dispatchEvent = function (domElement) {
                        var event = new Event('backgroundReady');
                        domElement.dispatchEvent(event);
                    };

                self._paintCanvas( self.myCanvas, self.domElement.querySelector('.js-require-backgroundImage__secondBackground') );
    ...
    backgroundImage.prototype = {
        ...
        applyFilters: function ( myCanvas, parameters, noiseMap ) {
            var tmpCanvas = helpers.newCanvas( parameters.width, parameters.height );
            parameters.filters.forEach( function (filter) {
                var filterName = filter['name'];
                if ( helpers.filters.hasOwnProperty(filterName) ) {
                    helpers.filters[filterName]( myCanvas, filter, noiseMap, tmpCanvas );
                }
            });
        }, // applyFilters()

        _createNoisemap: function (width, height) {
            var noiseMap = {};
        ...
        _zufall: function (noiseMap) {
            for (var i = 0; i < noiseMap.width; i++ ) {
                for (var j = 0; j < noiseMap.height; j++ ) {
                    noiseMap.imgData[i][j] = Math.floor(Math.random()*255); //a
                }
            }
        }, // zufall()
